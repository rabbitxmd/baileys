...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            const isStatusWrapperMessage = !!(message?.groupStatusMessageV2 ||
                message?.groupStatusMessage ||
                message?.statusMentionMessage ||
                message?.groupStatusMentionMessage);
            const innerMessage = isStatusWrapperMessage ? message : normalizeMessageContent(message);

            // Extract the inner message from inside the status wrapper (one level deep),
            // so media types (audio/image/video/etc) can be detected correctly.
            const wrappedInner = isStatusWrapperMessage
                ? (message.groupStatusMessageV2?.message ||
                   message.groupStatusMessage?.message ||
                   message.statusMentionMessage?.message ||
                   message.groupStatusMentionMessage?.message)
                : undefined;

            const mediaType = getMediaType(wrappedInner || innerMessage);
            if (mediaType) {
                extraAttrs['mediatype'] = mediaType;
            }

            // Text status must stay type="text" (the original fix for that bug),
            // but media status (audio/image/video/...) must be type="media"
            // with the correct mediatype, or WhatsApp won't play it back correctly.
            const messageTypeSource = (isStatusWrapperMessage && mediaType) ? wrappedInner : innerMessage;
            ...
            if (isNewsletter) {
                ...
                const stanza = {
                    ...
                    attrs: {
                        to: jid,
                        id: msgId,
                        type: getMessageType(messageTypeSource),
                        ...(additionalAttributes || {})
                    },
    ...
    /**
     * One entry point for "Full DP" (uncropped profile picture) updates.
     * Auto-detects the jid type and routes to the right underlying call:
     * - `@newsletter` jid  -> channel picture
     * - `@g.us` jid        -> group picture
     * - anything else      -> your own / a contact's picture
     */
    const updateFullDp = async (jid, content, dimensions) => {
        if (isJidNewsletter(jid)) {
            return await sock.newsletterUpdateFullPicture(jid, content);
        }
        if (isJidGroup(jid)) {
            return await sock.updateFullGroupProfilePicture(jid, content, dimensions);
        }
        return await sock.updateFullProfilePicture(jid, content, dimensions);
    };
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                // wbails merge --- special message types (payment, product, album, event, poll result, order, group story/label) via luxu.js
                ...
                // Lia@Changes 16-08-26 --- Add "statusMentions" support for normal status@broadcast sends: group jid auto-expands to a group mention (+ its members added to statusJidList), any other jid becomes a user mention
                let statusMentionJids;
                if (jid === 'status@broadcast' && Array.isArray(options.statusMentions) && options.statusMentions.length) {
                    statusMentionJids = [...new Set(options.statusMentions)];
                    const expandedUsers = new Set(options.statusJidList || []);
                    for (const id of statusMentionJids) {
                        if (isJidGroup(id)) {
                            try {
                                const groupData = (cachedGroupMetadata ? await cachedGroupMetadata(id) : null) || await groupMetadata(id);
                                for (const participant of groupData.participants) {
                                    expandedUsers.add(participant.id);
                                }
                            }
                            catch (error) {
                                logger.error(`Error getting metadata group from ${id}: ${error}`);
                    ...
                    options = { ...options, statusJidList: Array.from(expandedUsers) };
                }
                ...
                // Lia@Changes 16-08-26 --- Send the individual group/user status mention notifications for "statusMentions"
                if (statusMentionJids) {
                    const { delayMs = 1500 } = options;
                    for (const id of statusMentionJids) {
                        const isGroup = isJidGroup(id);
                        const sendType = isGroup ? 'groupStatusMentionMessage' : 'statusMentionMessage';
