/**
 * Voip module — vendored from baileys-caller (github.com/SheIITear/baileys-caller),
 * adapted to ATTACH to an already-connected sock from this Baileys fork instead of
 * creating its own auth/QR/socket. No separate session, no separate dependency.
 *
 * Usage (after your bot's sock is open):
 *
 *   import { attachVoip } from '@whiskeysockets/baileys' // or relative import
 *   const voip = await attachVoip(sock)
 *   const call = await voip.call('8801xxxxxxxxx', { audioSource: './song.mp3' })
 *   call.on('connected', () => console.log('call answered'))
 *   call.on('ended', (reason) => console.log('ended:', reason))
 *
 * Original signaling/wasm-engine/relay-transport/audio-feeder logic is untouched —
 * only the connection bootstrap in this file was changed.
 */
...
/**
 * Voice-call layer bound to an existing, already-connected Baileys `sock`.
 * Does NOT own the socket: never creates auth state, never scans a QR,
 * never closes the sock on disconnect() — it just rides along.
 */
export class VoipClient {
    ...
    /** Bind to an existing, already-open Baileys sock. Call once per sock. */
    attach = async (sock) => {
        if (this.#ready)
            return;
        this.#sock = sock;
    ...
    /** Place an outbound voice call over the attached sock. */
    call = async (phoneNumber, opts = {}) => {
        if (!this.#ready || !this.#engine || !this.#signaling)
            throw new Error("VoipClient not attached. Call attach(sock) first.");
        if (this.#activeCall)
    ...
    /** True if a call is currently active. */
    get busy() { return !!this.#activeCall; }

    /** Release engine/relay resources. Does NOT touch the shared sock. */
    disconnect = () => {
...
/**
 * Convenience helper: create a VoipClient and attach it to an already-open
 * sock in one call.
 *
 *   const voip = await attachVoip(sock)
 */
export const attachVoip = async (sock) => {
