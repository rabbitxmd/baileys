...
import { generateFullProfilePicture, generateProfilePicture } from '../Utils/messages-media.js';
...
// rabbitxmd@Changes --- Extra channel (newsletter) helpers ported from elaina-baileys
const NEWSLETTER_REACTION_SETTINGS = new Set(['ALL', 'BASIC', 'NONE', 'BLOCKLIST']);
const findChildByTag = (node, tag) => {
...
const extractNewsletterMessageMeta = (stanza) => {
    ...
    const editTimestamp = readIntAttr(meta.attrs, 'msg_edit_t');
    if (editTimestamp !== undefined) result.editTimestamp = editTimestamp;
    const originalTimestamp = readIntAttr(meta.attrs, 'original_msg_t');
...
export const makeNewsletterSocket = (config) => {
    ...
    // rabbitxmd@Changes --- my_addons fetcher (used by newsletterMyAddOns / newsletterStatusMyAddOns), ported from elaina-baileys
    ...
    // rabbitxmd@Changes --- Channel (newsletter) status: send/react/revoke/fetch native @newsletter statuses
    const newsletterSendStatus = makeNewsletterStatusSender(sock, config);
    const newsletterReactStatus = makeNewsletterStatusReactionSender(sock);
    const newsletterRevokeStatus = makeNewsletterStatusRevokeSender(sock);
    const newsletterFetchStatuses = makeNewsletterStatusFetcher(sock);
    const newsletterFetchStatusUpdates = makeNewsletterStatusUpdatesFetcher(sock);
    return {
        ...sock,
        executeWMexQuery,
        newsletterSendStatus,
        newsletterReactStatus,
        newsletterRevokeStatus,
        newsletterFetchStatuses,
        newsletterFetchStatusUpdates,
        ...
        /** Set the FULL (uncropped) channel/newsletter picture */
        newsletterUpdateFullPicture: async (jid, content) => {
            const { img } = await generateFullProfilePicture(content);
            return await newsletterUpdate(jid, { picture: img.toString('base64') });
        },
        ...
        // rabbitxmd@Changes --- Remaining channel features ported from elaina-baileys
        newsletterUpdateUserSetting: async (jid, type, value) => {
            ...
            const response = await executeWMexQuery({
                input: { newsletter_id: jid, type: setting, value: value ? 'ON' : 'OFF' }
        ...
        newsletterMyAddOns: async (options = {}) => fetchMyAddOns(options, undefined),
        newsletterStatusMyAddOns: async (options = {}) => fetchMyAddOns(options, 'status'),
                ...
                adminProfile: response?.admin_profile
                    ? {
                        ...
                        picture: response.admin_profile.picture
                            ? { id: response.admin_profile.picture.id, directPath: response.admin_profile.picture.direct_path }
                            : undefined
                    }
                    : undefined,
        ...
        newsletterEnforcements: async (jid, locale = 'en_US') => {
            ...
            const mapBase = (entry) => ({
                ...
                appealReasonOptions: (entry?.appeal_reason_options ?? []).map((option) => ({ reason: option?.reason, label: option?.label })),
        ...
        newsletterPollVoters: async (jid, serverId, options = {}) => {
            return executeWMexQuery({
                input: { newsletter_id: jid, server_id: String(serverId), limit: options.limit ?? 100, vote_hash: options.voteHash }
            }, QueryIds.POLL_VOTERS, XWAPaths.voter_list);
        },
        newsletterReactionSenders: async (jid, serverId) => {
            return executeWMexQuery({ input: { id: jid, server_id: String(serverId) } }, QueryIds.REACTION_SENDER_LIST, XWAPaths.xwa2_newsletters_reaction_sender_list);
        },
        ...
        newsletterLabelAiContent: async (jid, serverId, messageType = 'MESSAGE') => {
            return executeWMexQuery({ newsletter_id: jid, server_id: String(serverId), message_type: messageType }, QueryIds.LABEL_AI_CONTENT, XWAPaths.xwa2_newsletter_label_ai_content);
        },
        newsletterLabelPaidPartnership: async (jid, serverId, messageType = 'MESSAGE') => {
            return executeWMexQuery({ newsletter_id: jid, server_id: String(serverId), message_type: messageType }, QueryIds.PAID_PARTNERSHIP_LABEL, XWAPaths.xwa2_newsletter_label_paid_partnership);
        },
        ...
        newsletterDirectoryList: async (options = {}) => {
            return executeWMexQuery({
                ...
                input: {
                    ...
                    filters: { country_codes: options.countryCodes ?? [], categories: options.categories ?? [] },
        ...
        newsletterDirectorySearch: async (searchText, options = {}) => {
            return executeWMexQuery({
                fetch_status_metadata: options.fetchStatusMetadata ?? false,
                input: { search_text: searchText, categories: options.categories ?? [], limit: options.limit ?? 20, start_cursor: options.cursorToken }
            }, QueryIds.DIRECTORY_SEARCH, XWAPaths.xwa2_newsletters_directory_search);
        },
        newsletterDirectoryCategories: async (options = {}) => {
            return executeWMexQuery({
                fetch_status_metadata: options.fetchStatusMetadata ?? false,
                input: { categories: options.categories ?? [], country_code: options.countryCode || undefined, per_category_limit: options.perCategoryLimit ?? 10 }
        ...
        newsletterSendPollVote: async (jid, parentServerId, options) => {
            ...
            await query({
                tag: 'message',
                attrs: { to: jid, id: messageId, type: 'poll', server_id: String(parentServerId) },
                content: [
        ...
        newsletterInsights: async (jid, options = {}) => {
            return executeWMexQuery({ input: { newsletter_id: jid, metrics: options.metrics ?? ['NET_FOLLOWS', 'UNFOLLOWS'] } }, QueryIds.INSIGHTS, XWAPaths.xwa2_newsletter_admin_insights);
        },
        newsletterFollowers: async (jid, options = {}) => {
            return executeWMexQuery({ input: { newsletter_id: jid, count: options.count ?? 100 } }, QueryIds.FOLLOWERS, XWAPaths.xwa2_newsletter_followers);
        },
        ...
        newsletterQuestionResponseState: async (jid, serverId, responseServerId, state) => {
            return executeWMexQuery({
                newsletter_id: jid, server_id: String(serverId), response_server_id: String(responseServerId), state
        ...
        newsletterRecommended: async (options = {}) => {
            return executeWMexQuery({
                fetch_status_metadata: options.fetchStatusMetadata ?? false,
                input: { limit: options.limit ?? 20, country_codes: options.countryCodes ?? [] }
            }, QueryIds.RECOMMENDED, XWAPaths.xwa2_newsletters_recommended);
        },
        newsletterSimilar: async (jid, options = {}) => {
            return executeWMexQuery({
                fetch_status_metadata: options.fetchStatusMetadata ?? false,
                input: { newsletter_id: jid, limit: options.limit ?? 20, country_codes: options.countryCodes ?? [] }
