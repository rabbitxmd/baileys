...
const loadBaileys = async () => {
    ...
    try {
        ...
        throw new Error("Could not import this Baileys fork's lib/index.js from lib/Voip/dist/signaling.mjs.");
    }
};
...
export class SignalingBridge {
    constructor(config) {
                ...
                this._sock.ev.on("chats.update", updates => {
                    for (const update of updates || []) {
                        if (update?.id && update?.tcToken instanceof Uint8Array && update.tcToken.length > 0) {
                            this._log("[RTC TC TOKEN FROM CHATS.UPDATE]", {
                                jid: update.id,
            ...
            if (this._sock.ws?.on) {
                const handlePrivacyNotification = node => {
                    try {
                        ...
                        this._log("[RTC RAW PRIVACY TOKEN ERROR]", err?.message || err);
                    }
                };
        ...
        this.sendSignaling = (peerJid, callId, xmlPayload) => {
            this._outgoingSignalingQueue = this._outgoingSignalingQueue
                ...
                .catch(err => {
                    this._log("[RTC SIGNALING ERROR]", err?.stack || err?.message || err);
                });
        };
        ...
        this.processIncomingCall = (node, voip, activeCallId) => {
            this._incomingSignalingQueue = this._incomingSignalingQueue
                ...
                .catch(err => {
                    this._log("[RTC INCOMING CALL ERROR]", err?.stack || err?.message || err);
                });
        };
        ...
        this.processIncomingReceipt = (node, voip, activeCallId) => {
            this._incomingSignalingQueue = this._incomingSignalingQueue
                ...
                .catch(err => {
                    this._log("[RTC INCOMING RECEIPT ERROR]", err?.stack || err?.message || err);
                });
        };
        ...
        this.requestTcToken = async jid => {
            ...
            const logResponse = (label, response) => {
                this._log(`[RTC TC TOKEN RESPONSE:${label}]`, JSON.stringify(response, (key, value) => {
            ...
            for (const attempt of attempts) {
                try {
                    ...
                    this._log(`[RTC TC TOKEN REQUEST ERROR:${attempt.label}]`, err?.stack || err?.message || err);
                }
        ...
        this.discoverPeerDevices = async peerJid => {
            ...
            for (const [jid, useCache, ignoreZeroDevices] of attempts) {
                try {
                    ...
                    if (list.length) {
                        this._log("[RTC DEVICES]", { peerJid: bareJid, list });
                        return list;
                    }
                } catch (err) {
                    this._log("[RTC DEVICES ERROR]", err?.message || err);
                }
            }

            const fallback = [`${decoded.user}:0@${server}`];
            this._log("[RTC DEVICES FALLBACK]", { peerJid: bareJid, fallback });
            return fallback;
        };
        ...
        this.issueTcToken = async jid => {
            ...
            try {
                ...
                this._log("[RTC ISSUE TC TOKEN ERROR]", err?.message || err);
                return false;
            }
        };
        ...
        this._resolveLidViaUSync = async pnJid => {
            ...
            try {
                ...
                if (lid) {
                    this._log("[RTC LID RESOLVED]", { pn: pnJid, lid });
                    return lid;
                }
            } catch (err) {
                this._log("[RTC RESOLVE LID USYNC ERROR]", err?.message || err);
            }
            ...
            if (destination) {
                ...
                for (const destNode of destinations) {
                    ...
                    try {
                        ...
                        this._log("[RTC ENCRYPT DEST ERROR]", err?.message || err);
        ...
        this._sendCallStanza = async (routeTo, voipNode, signalingTag, effectivePeerJid, callbackPeerJid) => {
            ...
            this._log("[RTC SEND CALL STANZA]", { to: routeTo, id: stanzaId, tag: signalingTag, peer: effectivePeerJid });
            ...
            void (async () => {
                try {
                    if (typeof this._sock.waitForMessage !== "function") {
                        this._log("[RTC CALL ACK] waitForMessage not available");
                        return;
                    }
                    const ackNode = await this._sock.waitForMessage(stanzaId, ACK_TIMEOUT_MS);
                    this._log("[RTC CALL ACK]", ackNode?.attrs || null);
                    ...
                    try {
                        ...
                        this._log("[RTC HANDLE ACK ERROR]", err?.message || err);
                    }
                } catch (err) {
                    this._log("[RTC WAIT ACK ERROR]", err?.message || err);
                }
            })();
        };
    ...
    if (typeof this._sock.assertSessions === "function") {
        try {
            ...
            this._log("[RTC ASSERT SESSIONS ERROR]", err?.message || err);
        }
    }
    ...
    try {
        ...
        this._log("[RTC ENSURE SIGNAL SESSION ERROR]", {
        ...
        this._processPrivacyTokenNotification = node => {
            ...
            for (const tokenNode of tokenNodes) {
                ...
                if (jid && content instanceof Uint8Array && content.length > 0) {
                    this._log("[RTC TC TOKEN FROM RAW NOTIFICATION]", {
                        jid,
