...
export const makeSocket = (config) => {
    ...
    const toPn = async lid => {
        ...
        for (const l of lids) {
            ...
            if (!decoded)
                continue;
            usyncQuery.withUser(new USyncUser()
    ...
    const toLID = async pn => {
        ...
        for (const p of pns) {
            ...
            if (!decoded)
                continue;
            usyncQuery.withUser(new USyncUser().withId(jidEncode(decoded.user, 's.whatsapp.net')));
        }
        const result = await executeUSyncQuery(usyncQuery);
        return pns.map(p => {
            const decoded = jidDecode(p);
    ...
    /**
     * @param {string | string[]} username - username to fetch, pin - pin of username if the user enabled it
