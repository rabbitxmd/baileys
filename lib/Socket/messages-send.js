...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            // Lia@Changes 16-08-26 --- Status wrapper types (groupStatusMessageV2/groupStatusMessage/statusMentionMessage/groupStatusMentionMessage) are FutureProofMessage-style wrappers that WhatsApp expects as type="text" with no mediatype attr. Normalizing them here was making media (e.g. audio) sent as a group/newsletter status render as a generic "Update" placeholder instead of playing correctly, so skip normalization for those and keep them raw like the rest of the stanza expects.
