...
// Lia@Changes 16-08-26 --- Small helpers for the newer status/question/scheduled-call message builders below
const requireModernObject = (value, name) => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new TypeError(`${name} must be an object`);
    }
    return value;
};
const requireModernKey = (value, name = 'key') => {
    requireModernObject(value, name);
    if (!value.id) {
        throw new TypeError(`${name}.id is required`);
    }
    return value;
};
const modernEnumValue = (enumObject, value, fallback) => {
...
const toModernMilliseconds = value => value instanceof Date ? value.getTime() : value;
/**
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: rabbitxmd', description = '🏷️ rabbitxmd/baileys' } = message;
...
// Lia@Changes 16-08-26 --- Add makeNewsletterStatusAttribution() to build a StatusAttribution(NEWSLETTER_STATUS) proto for reshared channel posts
export const makeNewsletterStatusAttribution = (input) => {
    const data = requireModernObject(input, 'newsletterStatus');
...
// Lia@Changes 16-08-26 --- Add makeGroupStatusAttribution() to build a StatusAttribution(GROUP_STATUS) proto
export const makeGroupStatusAttribution = (input) => {
    if (!input || typeof input !== 'object') {
        throw new TypeError('groupStatusAttribution must be an object');
    }
    if (!input.authorJid) {
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Changes 16-08-26 --- Add "groupStatusReaction" to send a reaction onto a group status
    else if (hasNonNullishProperty(message, 'groupStatusReaction')) {
        const data = message.groupStatusReaction;
        if (!data || typeof data !== 'object') {
            throw new TypeError('groupStatusReaction must be an object');
        }
        if (!data.key) {
            throw new TypeError('groupStatusReaction.key is required');
        }
    ...
    // Lia@Changes 16-08-26 --- Add "question" to post a status question box
    else if (hasNonNullishProperty(message, 'question')) {
        const data = requireModernObject(message.question, 'question');
        if (typeof data.text !== 'string' || !data.text.length) {
            throw new TypeError('question.text is required');
        }
        m.questionMessage = proto.Message.FutureProofMessage.create({
            message: {
    ...
    // Lia@Changes 16-08-26 --- Add "questionResponse" to answer a status question box
    else if (hasNonNullishProperty(message, 'questionResponse')) {
        const data = requireModernObject(message.questionResponse, 'questionResponse');
        requireModernKey(data.key, 'questionResponse.key');
        m.questionResponseMessage = proto.Message.QuestionResponseMessage.create({
            key: data.key,
            text: data.text ?? ''
        });
    }
    // Lia@Changes 16-08-26 --- Add "statusQuestionAnswer" to reply to a status question with a plain text answer
    else if (hasNonNullishProperty(message, 'statusQuestionAnswer')) {
        const data = requireModernObject(message.statusQuestionAnswer, 'statusQuestionAnswer');
        requireModernKey(data.key, 'statusQuestionAnswer.key');
        m.statusQuestionAnswerMessage = proto.Message.StatusQuestionAnswerMessage.create({
            key: data.key,
            text: data.text ?? ''
        });
    }
    // Lia@Changes 16-08-26 --- Add "statusQuoted" to build a new status that quotes an existing one
    else if (hasNonNullishProperty(message, 'statusQuoted')) {
        const data = requireModernObject(message.statusQuoted, 'statusQuoted');
        if (!data.originalStatusId) {
            throw new TypeError('statusQuoted.originalStatusId is required');
        }
        const type = modernEnumValue(proto.Message.StatusQuotedMessage.StatusQuotedMessageType, data.type, proto.Message.StatusQuotedMessage.StatusQuotedMessageType.QUESTION_ANSWER);
        m.statusQuotedMessage = proto.Message.StatusQuotedMessage.create({
            type,
            text: data.text,
    ...
    // Lia@Changes 16-08-26 --- Add "statusStickerInteraction" to react to a status with a sticker
    else if (hasNonNullishProperty(message, 'statusStickerInteraction')) {
        const data = requireModernObject(message.statusStickerInteraction, 'statusStickerInteraction');
        requireModernKey(data.key, 'statusStickerInteraction.key');
        if (!data.stickerKey) {
            throw new TypeError('statusStickerInteraction.stickerKey is required');
        }
        const type = modernEnumValue(proto.Message.StatusStickerInteractionMessage.StatusStickerType, data.type, proto.Message.StatusStickerInteractionMessage.StatusStickerType.REACTION);
        m.statusStickerInteractionMessage = proto.Message.StatusStickerInteractionMessage.create({
            key: data.key,
            stickerKey: data.stickerKey,
            type
        });
    }
    // Lia@Changes 16-08-26 --- Add "scheduledCall" to send a scheduled-call creation message
    else if (hasNonNullishProperty(message, 'scheduledCall')) {
        const data = requireModernObject(message.scheduledCall, 'scheduledCall');
        if (!data.scheduledTimestampMs) {
            throw new TypeError('scheduledCall.scheduledTimestampMs is required');
        }
        const callType = modernEnumValue(proto.Message.ScheduledCallCreationMessage.CallType, data.callType, proto.Message.ScheduledCallCreationMessage.CallType.UNKNOWN);
        m.scheduledCallCreationMessage = proto.Message.ScheduledCallCreationMessage.create({
            scheduledTimestampMs: toModernMilliseconds(data.scheduledTimestampMs),
            callType,
            title: data.title
        });
    }
    // Lia@Changes 16-08-26 --- Add "scheduledCallEdit" to cancel/edit an existing scheduled call
    else if (hasNonNullishProperty(message, 'scheduledCallEdit')) {
        const data = requireModernObject(message.scheduledCallEdit, 'scheduledCallEdit');
        requireModernKey(data.key, 'scheduledCallEdit.key');
        const editType = modernEnumValue(proto.Message.ScheduledCallEditMessage.EditType, data.editType, proto.Message.ScheduledCallEditMessage.EditType.CANCEL);
        m.scheduledCallEditMessage = proto.Message.ScheduledCallEditMessage.create({
            key: data.key,
            editType
        });
    }
    ...
    // Lia@Changes 16-08-26 --- Add "newsletterStatus" to attach a StatusAttribution(NEWSLETTER_STATUS) to contextInfo.statusAttributions
    if (hasOptionalProperty(message, 'newsletterStatus') && !!message.newsletterStatus) {
        const attribution = makeNewsletterStatusAttribution(message.newsletterStatus);
        const messageType = Object.keys(m)[0];
        const key = m[messageType];
        if (key) {
            const existing = (key.contextInfo && key.contextInfo.statusAttributions) || [];
            key.contextInfo = {
                ...(key.contextInfo || {}),
                statusAttributions: [...existing, attribution]
            };
        }
        delete message.newsletterStatus;
    }
    // Lia@Changes 16-08-26 --- Add "groupStatusAttribution" to attach a StatusAttribution(GROUP_STATUS) to contextInfo.statusAttributions
    if (hasOptionalProperty(message, 'groupStatusAttribution') && !!message.groupStatusAttribution) {
        const attribution = makeGroupStatusAttribution(message.groupStatusAttribution);
        const messageType = Object.keys(m)[0];
        const key = m[messageType];
        if (key) {
            const existing = (key.contextInfo && key.contextInfo.statusAttributions) || [];
        ...
        delete message.groupStatusAttribution;
    }
