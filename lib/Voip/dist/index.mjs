/**
 * VoIP client: voice/video calls, group calls, and call-audio/video
 * playback, layered on top of an already-connected Baileys `sock`.
 * Does NOT own the socket: never creates auth state, never scans a QR,
 * never closes the sock unless it opened it itself.
 *
 * @author ShellTear
 */
...
import { VideoFeeder, VIDEO_FORMAT_I420 } from './video-feeder.mjs';
...
export class VoipClient {
    constructor(config = {}) {
        ...
        this.connect = async () => {
            if (!this._sock) {
                throw new Error('VoipClient needs the socket you are already logged in with: new VoipClient({ socket: sock }), or call attach(sock) instead. It never opens a session of its own, so there is no second pairing or QR scan.');
            }
            if (!this._sock.ws) {
        ...
        /**
         * Bind to a Baileys sock. First call on a fresh client performs the
         * full VoIP handshake (equivalent to connect()); calling it again
         * (e.g. after a reconnect) just rebinds the socket and its handlers.
         */
        this.attach = async socket => {
            if (!socket) {
                throw new Error('attach needs a socket');
            }
            if (!this._engine) {
                this._sock = socket;
...
export { VideoFeeder, VIDEO_FORMAT_I420 } from './video-feeder.mjs';
