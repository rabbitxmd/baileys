...
RabbitXmd ships with a fluent, chainable message-builder toolkit for buttons, carousels, and rich AI-style responses — no need to hand-craft raw payloads.

```javascript
import { Button, ButtonV2, Carousel, AIRich } from '@whiskeysockets/baileys'
```
...
const btn = new Button(sock)
   ...
   .setFooter('RabbitXmd Bot')
   .addReply('📄 View Menu', 'menu_id')
   .addUrl('🌐 Visit Website', 'https://example.com')
   .addCall('📞 Call Us', '+11234567890')
   .addCopy('📋 Copy Code', 'PROMO2026')

await btn.send(jid)
```

### 🔘 Button V2 (list-style)

```javascript
const btnV2 = new ButtonV2(sock)
   .setTitle('Pick a plan')
   .addButton('Basic', 'plan_basic')
   .addButton('Pro', 'plan_pro')
...
const carousel = new Carousel(sock)
   .addCard({
      ...
      buttons: [{ label: 'Buy Now', id: 'buy_a' }]
   })
   .addCard({
      image: { url: './product2.jpg' },
      title: 'Product B',
      body: 'Description here',
      buttons: [{ label: 'Buy Now', id: 'buy_b' }]
   })
...
For building chatbot-style responses with tables, code blocks, citations, and progress indicators:

```javascript
const rich = new AIRich(sock)
   .addText('Here is what I found:')
   .addCodeBlock('console.log("hello world")', 'javascript')
   .addTable(['Name', 'Score'], [['Alice', '95'], ['Bob', '88']])
   .addSource('https://example.com', 'Example Source')

const sent = await rich.send(jid)

// edit a previously sent rich message
await rich.addText('\n\nUpdated info:').sendEdit(jid, sent.key)
```

### 🧠 Extra Sections (HTML, Bloks, status indicators)

```javascript
import { sendHtmlDocument, thinkingSection, progressSection } from '@whiskeysockets/baileys'

// send a rendered HTML page as a document
await sendHtmlDocument(sock, jid, '<h1>Report</h1><p>Generated by RabbitXmd</p>')
```

> ℹ️ Some builder features (image resizing, video thumbnail previews) rely on the optional packages `sharp` and `fluent-ffmpeg`. Install them if you plan to use those features: `npm i sharp fluent-ffmpeg`
