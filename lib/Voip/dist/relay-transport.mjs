/**
 * Relay/ICE transport for the VoIP WASM engine.
 *
 * @author ShellTear
 */
...
const getRelayLookupId = (ip, port) => getConnectionIdentifier(
    ip,
    port === TRUE_WEB_CLIENT_RELAY_PORT ? TRUE_WEB_CLIENT_RELAY_PORT : port
);
...
const replaceDtlsFingerprint = (sdp, algorithm, fingerprint) => sdp
...
export class RelayRtcTransport {
    constructor(config) {
        this.config = config;
        this._relayInfoById = new Map();
        this._connections = new Map();
        this._totals = createEmptyStats();
        this._wrtcPromise = null;
        ...
        this.updateRelayList = update => {
            ...
            for (const id of this._relayInfoById.keys()) {
                if (!nextInfoById.has(id)) {
                    this._closeConnection(id);
                }
            }

            this._relayInfoById.clear();

            for (const [id, info] of nextInfoById) {
                this._relayInfoById.set(id, info);
                void this._ensureConnection(info);
            }
        };
        ...
        this.send = (packet, ip, port) => {
            ...
            for (const candidateId of candidateIds) {
                info = this._relayInfoById.get(candidateId);
            ...
            if (!info) {
                ...
                const earlyConnection = this._getOrCreateEarlyConnection(ip, port);
                bufferPacket(earlyConnection, toArrayBuffer(packet));

                return packet.byteLength;
            }

            const connection = this._getOrCreateConnection(info);
            ...
            if (
                ...
                void this._restartIce(connection);

                return packet.byteLength;
            }

            if (connection.state === "open" && connection.dataChannel?.readyState === "open") {
                if (!this._sendBufferedPacket(connection, arrayBuffer)) {
                    connection.stats.droppedPackets += 1;
                }
        ...
        this._getOrCreateConnection = info => {
            const existing = this._connections.get(info.id);
            ...
            this._connections.set(info.id, created);

            return created;
        };

        this._getOrCreateEarlyConnection = (ip, port) => {
            ...
            this._connections.set(id, created);

            return created;
        };

        this._ensureConnection = async info => {
            const connection = this._getOrCreateConnection(info);
            ...
            const promise = this._connect(connection);
        ...
        this._connect = async connection => {
            const wrtcModule = await this._loadWrtc();
            ...
            this._closePeerObjects(connection);
            ...
            dc.onopen = () => {
                ...
                this._flushBufferedPackets(connection);
                this._startIceRttPolling(connection);
            };
            ...
            dc.onmessage = event => {
                this._handleIncomingPacket(connection, event.data);
            };
        ...
        this._restartIce = async connection => {
            if (connection.isReconnecting || !connection.hasNonStunPacketSent) {
                return;
            }

            const { RTCPeerConnection } = await this._loadWrtc();
        ...
        this._handleIncomingPacket = (connection, raw) => {
            ...
            this._totals.receivedPackets += 1;
            this._totals.receivedBytes += packet.byteLength;
        ...
        this._flushBufferedPackets = connection => {
            while (
                ...
                if (!this._sendBufferedPacket(connection, packet)) {
                    connection.stats.droppedPackets += 1;
                    break;
                }
            }
        ...
        this._sendBufferedPacket = (connection, packet) => {
            try {
                ...
                this._totals.sentPackets += 1;
                this._totals.sentBytes += packet.byteLength;
        ...
        this._startIceRttPolling = connection => {
            this._stopIceRttPolling(connection);
            ...
            const poll = async () => {
                const rttMs = await this._readCurrentRoundTripTimeMs(pc);
        ...
        this._readCurrentRoundTripTimeMs = async pc => {
            try {
                ...
                const selectedPair =
                    selectedCandidatePairId && candidatePairs.find(report => report?.id === selectedCandidatePairId) ||
                    candidatePairs.find(report => report?.selected || report?.nominated) ||
                    candidatePairs[0];
        ...
        this._loadWrtc = () => {
            this._wrtcPromise ??= import("@roamhq/wrtc").then(module => module.default ?? module);

            return this._wrtcPromise;
        };
    }
}
